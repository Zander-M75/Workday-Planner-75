...
function colorCoding(row) {
  var present = dayjs();
  var rowTime = dayjs(present);

  rowTime.set({ hour: row });

  if (present.isSame(rowTime, "hour")) {
    return "present";
  }
  else if (present.isAfter(rowTime, "hour")) {
    return "past"
  }
  else {
    return "future";
...
function getColor(hour) {
  var rows = $('#' + hour);
  var className = colorCoding(hour)
  rows.attr("class", "col-9 time-block " + className);
}
...
function loadData(id) {
  var content = localstorage.getItem("content" + id);
  $("#" + id).val(content)
}
